...
class ShopImpl: Shop {
    ...
    func listProductsByName(searchString: String) -> Set<String> {
        ...
        let filtered = products.filter { $0.value.name.contains(searchString) }
        
        for (_, product) in filtered {
            
            let sameName = filtered.filter { $0.value.name == product.name }
            
            if sameName.count > 1 {
                set.insert("\(product.producer) - \(product.name)")
            } else {
                set.insert(product.name)
            }
        }
        return Set(set.prefix(10))
    }
    
    func listProductsByProducer(searchString: String) -> [String] {
        var array: [String] = []
        
        products.filter({ $0.value.producer.contains(searchString) })
